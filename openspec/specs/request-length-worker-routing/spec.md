...
Before policy scoring on every generation route, the gateway SHALL remove workers whose registered range does not contain the computed required context token budget. A request exactly equal to a lower bound SHALL be eligible, and a request exactly equal to an upper bound SHALL be eligible.

#### Scenario: Split a 64K pool
- **WHEN** AMD workers are registered with `@max_context_tokens=65535`, NVIDIA workers with `@min_context_tokens=65536`, and a request requires `65535` tokens
- **THEN** the router SHALL select only eligible AMD workers

#### Scenario: Route a request at or above the threshold
- **WHEN** the same pool receives a request requiring `65536` tokens or more
...
- **THEN** that worker SHALL remain eligible for known request lengths unless another independent eligibility filter removes it

### Requirement: Unknown request length fails closed for bounded workers
When the gateway cannot reliably compute the request's required context token budget, it SHALL exclude every worker with either request-length bound and SHALL continue only with unbounded workers.
...
- **WHEN** request length is unknown and every healthy candidate has a request-length bound
