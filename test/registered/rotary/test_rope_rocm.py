...
class TestRotaryEmbeddingAITer(CustomTestCase):
    ...
    def test_strided_positions_are_canonicalized(self) -> None:
        ...
        query = torch.randn(num_tokens, head_size, dtype=torch.bfloat16, device="cuda")
        key = torch.randn(num_tokens, head_size, dtype=torch.bfloat16, device="cuda")

        q_ref, k_ref = rope_ref.forward_native(positions, query.clone(), key.clone())
        q_hip, k_hip = rope_hip.forward_hip(positions, query.clone(), key.clone())
