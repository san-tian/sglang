...
class CpLayerSplitKVPool:
    ...
    def broadcast_owner_layer_prefix(
        ...
        # All CP ranks must reach the broadcast collective to avoid deadlock.
        # If this rank has no prefix_slots but other ranks do, we still need
        # to participate in the broadcast.  We use all_reduce(SUM) to get
        # the total count; if 0, all ranks skip; otherwise all ranks
        # allocate a buffer of size total_n.
        local_n = torch.tensor([len(prefix_slots_list)], dtype=torch.int32, device="cuda")
        # Use torch.distributed.all_reduce with MAX op (cp_group.all_reduce only
        # supports SUM). MAX ensures total_n == max(len(prefix_slots)) across
        # ranks, so normal-path ranks (n <= total_n) and dummy-path ranks
        # (buffer=total_n) use matching broadcast sizes.
        torch.distributed.all_reduce(local_n, op=torch.distributed.ReduceOp.MAX, group=cp_group.device_group)
        total_n = local_n.item()
        if total_n == 0:
            return
        if not prefix_slots_list:
            # This rank has no slots but others do.  Participate in broadcast
            # with a matching-size dummy buffer; result is discarded.
            if kind == "latent":
                ref = self.transient_pool.kv_buffer[0]
                dummy = ref.new_empty((total_n,) + ref.shape[1:])
                cp_group.broadcast(dummy, src=owner_rank)
            elif kind == "indexer":
        ...
        if kind == "latent":
            ref = self.transient_pool.kv_buffer[0]
            n = len(prefix_slots)
            # Use total_n as broadcast size to match dummy ranks
            bcast_n = max(n, total_n)
            if self._bcast_buf_latent is None or self._bcast_buf_latent.shape[0] < bcast_n:
                if self._bcast_buf_latent is not None: del self._bcast_buf_latent; torch.cuda.empty_cache()
                self._bcast_buf_latent = ref.new_empty((bcast_n,) + ref.shape[1:])
            buf = self._bcast_buf_latent[:bcast_n]
            row_indices = prefix_slots
            if my_rank == owner_rank:
                buf[:n].copy_(self.owned_pool.kv_buffer[owned_layer_idx][row_indices])
            recv = cp_group.broadcast(buf, src=owner_rank)
            if my_rank != owner_rank:
                self.transient_pool.kv_buffer[0][row_indices] = recv[:n]
            ...
            # Use total_n as broadcast size to match dummy ranks
            bcast_n = max(n, total_n)
            if self._bcast_buf_indexer is None or self._bcast_buf_indexer.shape[0] < bcast_n:
                if self._bcast_buf_indexer is not None: del self._bcast_buf_indexer; torch.cuda.empty_cache()
                self._bcast_buf_indexer = ref.new_empty((bcast_n,) + ref.shape[1:])
            buf = self._bcast_buf_indexer[:bcast_n]
            if my_rank == owner_rank:
                buf[:n].copy_(
            ...
            recv = cp_group.broadcast(buf, src=owner_rank)
            if my_rank != owner_rank:
                self.transient_pool.index_k_with_scale_buffer[0][row_indices] = recv[:n]
